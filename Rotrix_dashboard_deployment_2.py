    ...
    if st.session_state.files_submitted:
        ...
        if analysis_type == "Single File Analysis":
            ...
            with tab_plot:
                ...
                plot_col, param_col = st.columns([8.5, 2.6])
            ...
            if st.session_state.get('selected_bench') and st.session_state.get('selected_val'):
                ...
                with tab1:
                    ...
                    with param_col:
                        ...
                        with y_min_col:
                            y_min = st.number_input("Start", value=y_min_val, format="%.2f", key="y_min_comparative", step=1.0)
                        with y_max_col:
                            y_max = st.number_input("End", value=y_max_val, format="%.2f", key="y_max_comparative", step=1.0)
                        with y_reset_col:
                            st.markdown('<div style="margin-top: 28px;"></div>', unsafe_allow_html=True)
