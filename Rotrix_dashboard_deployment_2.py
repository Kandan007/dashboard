    ...
    if st.session_state.files_submitted:
        ...
        if analysis_type == "Single File Analysis":
            ...
            with col1:
                ...
                # Reset Y-axis and topic/column selection when switching files
                if selected_file != st.session_state.get("previous_file", "None"):
                    ...
                    # Clear topic/column selection state
                    if "ulg_topic_single" in st.session_state:
                        del st.session_state["ulg_topic_single"]
                    if "multi_col_select" in st.session_state:
                        del st.session_state["multi_col_select"]
            ...
            with col2:
                file_options_target = ["None"] + [f.name for f in st.session_state.uploaded_files if f.name != benchmark_file]
            ...
            with col3:
                topic_options = []
                topic_type = None
                if benchmark_file != "None" and target_file != "None":
                    bench_ext = os.path.splitext(benchmark_file)[-1].lower()
                    targ_ext = os.path.splitext(target_file)[-1].lower()
                    if bench_ext == targ_ext == ".ulg":
                        # ULG: topic selection
                        dfs_bench, topics_bench = load_ulog([f for f in st.session_state.uploaded_files if f.name == benchmark_file][0])
                        dfs_target, topics_target = load_ulog([f for f in st.session_state.uploaded_files if f.name == target_file][0])
                        common_topics = [a for _, a in TOPIC_ASSESSMENT_PAIRS if _ in dfs_bench and _ in dfs_target]
                        topic_options = common_topics
                        topic_type = "ulg"
                    elif bench_ext == targ_ext == ".csv":
                        # CSV: column selection
                        df_bench, _ = load_data([f for f in st.session_state.uploaded_files if f.name == benchmark_file][0], ".csv", "")
                        df_target, _ = load_data([f for f in st.session_state.uploaded_files if f.name == target_file][0], ".csv", "")
                        numeric_cols_bench = [col for col in df_bench.columns if pd.api.types.is_numeric_dtype(df_bench[col]) and col != 'timestamp_seconds']
                        numeric_cols_target = [col for col in df_target.columns if pd.api.types.is_numeric_dtype(df_target[col]) and col != 'timestamp_seconds']
                        topic_options = list(set(numeric_cols_bench) & set(numeric_cols_target))
                        topic_type = "csv"
                selected_topic = st.selectbox(
                    "Select Topic/Column",
                    ["None"] + topic_options,
                    key="comparative_topic_selector"
                ) if topic_options else None
            # Comparative plotting and data
            if benchmark_file != "None" and target_file != "None" and selected_topic and selected_topic != "None":
                bench_ext = os.path.splitext(benchmark_file)[-1].lower()
                targ_ext = os.path.splitext(target_file)[-1].lower()
                if bench_ext == targ_ext == ".ulg":
                    dfs_bench, _ = load_ulog([f for f in st.session_state.uploaded_files if f.name == benchmark_file][0])
                    dfs_target, _ = load_ulog([f for f in st.session_state.uploaded_files if f.name == target_file][0])
                    assessment_to_topic = {a: t for t, a in TOPIC_ASSESSMENT_PAIRS}
                    topic = assessment_to_topic.get(selected_topic)
                    if topic and topic in dfs_bench and topic in dfs_target:
                        df_bench = ensure_seconds_column(dfs_bench[topic].copy())
                        df_target = ensure_seconds_column(dfs_target[topic].copy())
                        y_cols = [col for col in ASSESSMENT_Y_AXIS_MAP.get(selected_topic, []) if col in df_bench.columns and col in df_target.columns]
                        if not y_cols:
                            st.info("No relevant columns found for this topic in both files.")
                            st.stop()
                        # Align time
                        min_time = max(df_bench['timestamp_seconds'].min(), df_target['timestamp_seconds'].min())
                        max_time = min(df_bench['timestamp_seconds'].max(), df_target['timestamp_seconds'].max())
                        df_bench = df_bench[(df_bench['timestamp_seconds'] >= min_time) & (df_bench['timestamp_seconds'] <= max_time)]
                        df_target = df_target[(df_target['timestamp_seconds'] >= min_time) & (df_target['timestamp_seconds'] <= max_time)]
                        # Plot
                        st.markdown("### 📊 Comparative Plot")
                        fig = go.Figure()
                        for y_col in y_cols:
                            fig.add_trace(go.Scatter(x=df_bench['timestamp_seconds'], y=df_bench[y_col], mode='lines', name=f"Benchmark {y_col}"))
                            fig.add_trace(go.Scatter(x=df_target['timestamp_seconds'], y=df_target[y_col], mode='lines', name=f"Target {y_col}"))
                        fig.update_layout(title=f"Comparative Analysis: {selected_topic}", xaxis_title="Time (secs)", yaxis_title=", ".join(y_cols))
                        st.plotly_chart(fig, use_container_width=True)
                        st.markdown("### 📋 Data Table (Benchmark)")
                        st.dataframe(df_bench, use_container_width=True)
                        st.markdown("### 📋 Data Table (Target)")
                        st.dataframe(df_target, use_container_width=True)
                elif bench_ext == targ_ext == ".csv":
                    df_bench, _ = load_data([f for f in st.session_state.uploaded_files if f.name == benchmark_file][0], ".csv", "")
                    df_target, _ = load_data([f for f in st.session_state.uploaded_files if f.name == target_file][0], ".csv", "")
                    if selected_topic in df_bench.columns and selected_topic in df_target.columns:
                        df_bench = ensure_seconds_column(df_bench)
                        df_target = ensure_seconds_column(df_target)
                        # Use index as x-axis
                        df_bench['Index'] = df_bench.index
                        df_target['Index'] = df_target.index
                        st.markdown("### 📊 Comparative Plot")
                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=df_bench['Index'], y=df_bench[selected_topic], mode='lines', name="Benchmark"))
                        fig.add_trace(go.Scatter(x=df_target['Index'], y=df_target[selected_topic], mode='lines', name="Target"))
                        fig.update_layout(title=f"Comparative Analysis: {selected_topic}", xaxis_title="Index", yaxis_title=selected_topic)
                        st.plotly_chart(fig, use_container_width=True)
                        st.markdown("### 📋 Data Table (Benchmark)")
                        st.dataframe(df_bench, use_container_width=True)
                        st.markdown("### 📋 Data Table (Target)")
                    ...
                    st.warning("Both files must be of the same type (.ulg or .csv) for comparative analysis.")
