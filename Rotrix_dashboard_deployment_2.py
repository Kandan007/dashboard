# type: ignore #Modified script to support single file analysis mode
    ...
    if st.session_state.files_submitted:
        ...
        with col1:
            analysis_type = st.radio(
                ...
                index=0 if st.session_state.get("analysis_type", "Single File Analysis") == "Single File Analysis" else 1,
                horizontal=True
            )
        with col2:
        ...
        if analysis_type == "Single File Analysis":
            ...
            if selected_bench != "None" and b_content:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=b_file_ext) as tmp_file:
                    ...
                    try:
                        if b_file_ext == ".ulg":
                            b_dfs, _ = load_ulog(tmp_file.name)
                        else:
            ...
            if selected_val != "None" and v_content:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=v_file_ext) as tmp_file:
                    ...
                    try:
                        if v_file_ext == ".ulg":
                            v_dfs, _ = load_ulog(tmp_file.name)
                        else:
            ...
            # For ULG files, select topic and extract DataFrames
            ...
            if st.session_state.get('selected_bench') and st.session_state.get('selected_val'):
                ...
                with tab1:
                    ...
                    default_y = None
                    if isinstance(b_df, pd.DataFrame) and isinstance(v_df, pd.DataFrame):
                        b_numeric = [col for col in b_df.columns if pd.api.types.is_numeric_dtype(b_df[col])]
                        v_numeric = [col for col in v_df.columns if pd.api.types.is_numeric_dtype(v_df[col])]
                        ...
                        if isinstance(selected_assessment, str) and selected_assessment != "None":
